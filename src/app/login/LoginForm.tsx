...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "login") {
      ...
      if (data.session) {
        ...
        setMessage({
          text: "Te enviamos un email de confirmación. Revisá tu bandeja de entrada para activar tu cuenta.",
          isInfo: true,
        });
        setLoading(false);
      }
  ...
  const handleForgotPassword = async () => {
    if (!form.email) {
      setMessage({ text: "Ingresá tu email para recuperar la contraseña." });
      return;
    }
    const supabase = createClient();
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "register" && (
              <>
                ...
                <div>
                  <label className="block text-sm font-medium text-graphite-700 mb-1.5">Empresa (opcional)</label>
            ...
            >
              {loading
                ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />Cargando...</span>
